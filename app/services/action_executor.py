...
class ActionExecutor:
    ...
    def execute_template_actions(self) -> Dict[str, Any]:
        ...
        for idx, action_seq in enumerate(execution_plan, 1):
            ...
            # Check if should pause BEFORE executing this action (for verify_ssh_key with show_modal)
            if self._should_pause_for_ssh_verification(action_seq):
                # Store current step before pausing (this action hasn't run yet)
                self.workspace.provisioning_step = idx
                db.session.commit()
                return self._pause_for_ssh_verification(action_seq, idx)
            ...
            if success:
                ...
                # Store SSH public key if this was generate_ssh_key action
                if action_seq.action_type == 'generate_ssh_key':
                    execution_result = execution.result or {}
                    if execution_result.get('public_key'):
                        self.workspace.ssh_public_key = execution_result['public_key']
                        current_app.logger.info(
                            f"Stored SSH public key for workspace {self.workspace.id}"
                        )
                ...
                # No longer check after execution - moved to before execution
            else:
    ...
    def _should_pause_for_ssh_verification(self, action_seq: TemplateActionSequence) -> bool:
        """
        Check if provisioning should pause BEFORE executing verify_ssh_key action.

        Pauses before verify_ssh_key if:
        - Action type is 'verify_ssh_key'
        - Action has 'show_modal: true' in parameters
        - Workspace doesn't have verified SSH already

        Args:
            action_seq: The action sequence to be executed

        Returns:
            True if should pause before executing this action
        """
        # Only check verify_ssh_key actions
        if action_seq.action_type != 'verify_ssh_key':
            return False

        # Check if this action requires modal
        parameters = action_seq.parameters or {}
        if not parameters.get('show_modal'):
            return False
        ...
        # Check if SSH public key exists (from previous generate_ssh_key action)
        if not self.workspace.ssh_public_key:
            return False

        return True

    def _pause_for_ssh_verification(self, action_seq: TemplateActionSequence, current_step: int) -> Dict[str, Any]:
        """
        Pause provisioning before executing verify_ssh_key action.

        Args:
            action_seq: The verify_ssh_key action that triggered the pause
            current_step: The step index where we paused (1-based)

        Returns:
            Dict indicating provisioning paused for SSH verification
        """
        # Create paused WorkspaceActionExecution record for the verify_ssh_key action
        # This ensures the action is tracked in the database for frontend display
        paused_execution = WorkspaceActionExecution(
            workspace_id=self.workspace.id,
            template_id=self.template.id,
            action_sequence_id=action_seq.id,
            action_id=action_seq.action_id,
            action_type=action_seq.action_type,
            status='paused',  # Mark as paused, not pending
            max_attempts=action_seq.retry_config.get('max_attempts', 1)
        )
        db.session.add(paused_execution)

        current_app.logger.info(
            f"✓ Created paused WorkspaceActionExecution for {action_seq.action_id}"
        )
        ...
        extra_data['paused_at_action'] = action_seq.action_id
        extra_data['paused_before_action'] = True  # Indicates we paused BEFORE this action
        ...
        current_app.logger.info(
            f"Provisioning paused for workspace {self.workspace.id} before {action_seq.action_id}"
        )
        ...
        return {
            ...
            'ssh_public_key': self.workspace.ssh_public_key,
            'paused_before_action': action_seq.action_id
        }
    ...
    def resume_from_current_step(self) -> Dict[str, Any]:
        ...
        # Check if we paused BEFORE executing an action
        extra_data = self.workspace.extra_data or {}
        paused_before_action = extra_data.get('paused_before_action', False)

        # If paused before action, resume from current step (action not yet executed)
        # Otherwise resume from next step (current action already completed)
        resume_from = current_step if paused_before_action else current_step + 1
        
        # Clear the paused_before_action flag
        if paused_before_action:
            extra_data['paused_before_action'] = False
            self.workspace.extra_data = extra_data
            db.session.commit()

        current_app.logger.info(
            f"Resuming from step {resume_from}/{total_steps} for workspace {self.workspace.id} "
            f"(paused_before_action={paused_before_action})"
        )

        # Execute remaining actions
        for idx in range(resume_from - 1, total_steps):
            ...
            # Check if this action has a paused execution record (from previous pause)
            # If so, mark it as completed and skip re-execution
            paused_exec = WorkspaceActionExecution.query.filter_by(
                workspace_id=self.workspace.id,
                action_id=action_seq.action_id,
                status='paused'
            ).first()
            ...
            if paused_exec:
                # This action was paused (e.g., verify_ssh_key after SSH setup)
                # Mark as completed since user has performed the verification
                paused_exec.mark_completed(
                    result={'verified': True, 'method': 'manual_verification'},
                    duration_seconds=0
                )
                db.session.commit()
                self.completed_actions.append(paused_exec)

                current_app.logger.info(
                    f"✓ Marked paused action {action_seq.action_id} as completed during resume"
                )
